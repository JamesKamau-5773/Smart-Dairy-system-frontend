...
export default function FinanceForm({ onSubmit }) {
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      <div className="rounded-2xl border border-ink/10 bg-[linear-gradient(135deg,rgba(235,248,255,0.94),rgba(255,255,255,0.98))] p-4 ">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-2xl border border-ink/10 bg-surface p-1 ">
        <button 
          type="button" 
          onClick={() => setType('Income')}
          className={`rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-widest transition-colors ${
            type === 'Income' ? 'bg-brand text-surface  : 'text-ink-muted hover:bg-ink/5 hover:text-ink'
          }`}
        >
          Income
        </button>
        <button 
          ...
          className={`rounded-xl px-4 py-2.5 text-xs font-bold uppercase tracking-widest transition-colors ${
            type === 'Expense' ? 'bg-danger text-surface  : 'text-ink-muted hover:bg-ink/5 hover:text-ink'
          }`}
        >
          Expense
        </button>
      ...
      <div className="grid gap-5 md:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-widest text-ink-muted">Amount (KES)</label>
      ...
      <div className="flex justify-end border-t border-ink/10 pt-2">
        <button type="submit" className="btn-command inline-flex items-center justify-center rounded-lg bg-brand px-5 py-2.5 text-sm font-semibold text-surface ">
        Save transaction
        </button>
      </div>
    </form>
